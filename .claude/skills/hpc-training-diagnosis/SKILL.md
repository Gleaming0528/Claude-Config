...
description: 提供分布式训练故障深度诊断方法论。适用于分析 NCCL 超时、GPU Xid 错误、训练 hang、多节点级联故障。
---
...
| 借口 | 现实 |
|------|------|
| "NCCL 超时就是网络问题" | NCCL 超时是症状不是根因。可能是 GPU 硬件、存储 hang、用户代码 bug、调度问题。必须按时间线排查。 |
| "重启任务就好了" | 间歇性故障不会自愈。重启掩盖了根因，下次还会出现，而且可能影响更多任务。 |
| "只看用户容器日志就够了" | 分布式训练故障链跨多个层：GPU → NCCL → 网络 → 存储 → 调度。必须交叉关联 K8s Event、节点日志、Grafana 指标。 |
| "单 rank 失败是用户代码问题" | 需要确认首个失败 rank。级联故障中，最先失败的 rank 才指向根因，后续 rank 都是连锁反应。 |
| "Xid 错误就是显卡坏了" | Xid 48（双比特 ECC）通常是硬件，但 Xid 13/31 可能是驱动 bug 或供电问题。需要看频率和模式。 |

## Red Flags

- 只看了一个 rank 的日志就下结论
- 没有建立时间线就开始分析
- 把 NCCL timeout 直接归因为"网络问题"
- 忽略了 K8s Event 和节点级事件
- 没有交叉关联不同来源的日志（容器、节点、Grafana）
- 故障复现但没有记录模式（哪些节点、什么时间、什么条件）
- 建议"重启试试"但没有找到根因
...
- [ ] 已建立完整时间线（首个异常事件 → 级联 → 最终失败）
- [ ] 已定位首个失败 rank 和对应节点
- [ ] 已交叉关联多个日志源（容器日志、K8s Event、Grafana 指标）
- [ ] 根因归类到具体模式（GPU 硬件 / 网络 / 存储 / 用户代码 / 调度）
- [ ] 有明确的处置建议（换节点、修代码、联系运维等）
- [ ] 如为硬件问题，已记录故障节点信息供运维排查
